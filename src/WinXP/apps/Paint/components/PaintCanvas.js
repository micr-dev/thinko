...
export const PaintCanvas = forwardRef(
  (
    ...
    const saveToHistory = useCallback(() => {
      ...
      if (!isLoadingFile) {
        onContentChange === null || onContentChange === void 0
          ? void 0
          : onContentChange();
      }
    }, [
        ...
        for (let y = 0; y < intHeight; y++) {
          for (let x = 0; x < intWidth; x++) {
            ...
            if (maskData.data[canvasIdx + 3] > 0) {
              ...
              outputData.data[outputIdx + 1] =
                fullImageData.data[canvasIdx + 1];
              outputData.data[outputIdx + 2] =
                fullImageData.data[canvasIdx + 2];
              outputData.data[outputIdx + 3] =
                fullImageData.data[canvasIdx + 3];
            } else {
              // Transparent pixel
      ...
      return new Promise((resolve, reject) => {
        ...
        tempCanvas.toBlob(blob => {
          ...
          if (!ClipboardItemCtor) {
            reject(new Error('ClipboardItem is not available'));
            return;
          }
    ...
    useEffect(() => {
      ...
      const handleClipboardShortcuts = e => {
        const isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;
    ...
    const getCanvasPoint = event => {
      ...
      if ('touches' in event) {
        ...
        if (
          ...
          ((_a = event.changedTouches) === null || _a === void 0
            ? void 0
            : _a.length) > 0
        ) {
    ...
    const isPointInLassoPath = useCallback((point, path) => {
    ...
    useEffect(() => {
      const canvas = canvasRef.current;
      if (!canvas) return;
      const touchStartHandler = e => handlePointerDown(e);
      const touchMoveHandler = e => handlePointerMove(e);
      const touchEndHandler = e => handlePointerUp(e);
        ...
        React.createElement(
          ...
          isTyping &&
            ...
            React.createElement('input', {
              ...
              type: 'text',
              className:
                'absolute bg-transparent border-none outline-none font-geneva-12 text-black pointer-events-auto',
              style: {
